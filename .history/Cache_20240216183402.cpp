...
int main(int argc, char *argv[])
{
    ...
    {
        std::vector<double> row; // Store hit rates for each associativity in a row
        ...
        for (unsigned int associativity : {1, 2, 4, 8})
        {
            ...
            // Store the hit rate in the row
            double hitRate = cache.getHitRate();
            row.push_back(hitRate);

            // Print the hit rate in the table
            std::cout << std::setw(15) << std::fixed << std::setprecision(4) << hitRate;
        }

        hitRates.push_back(row); // Store the row of hit rates for each size
