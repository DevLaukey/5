...
int main(int argc, char *argv[])
{
    ...
    for (unsigned int size : {2048, 4096, 8192, 16384, 32768})
    {
        ...
        // Store hit rates for each associativity in a row
        ...
        for (unsigned int associativity : {1, 2, 4, 8})
        {
            ...
            // Calculate and store the hit rate for the specific associativity
            double hitRate = static_cast<double>(totalHits) / totalAccesses;
            row.push_back(hitRate);
        }

        // Print hit rates for each associativity in the row
        for (const auto &hitRate : row)
        {
